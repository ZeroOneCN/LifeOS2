...
| 功能   | 说明                                                       |
| ---- | -------------------------------------------------------- |
| 用户认证 | 注册 / 登录，基于 JWT 的身份认证，首个注册账号自动成为管理员                       |
| 用户中心 | 个人资料与账户设置                                                |
| 系统设置 | 自定义系统 LOGO、系统标题及登录 / 注册页文案，仅管理员可改，全站与登录 / 注册页即时同步生效      |
| 数据备份 | 数据全量导出 / 导入、定时备份计划与备份渠道管理                                |
| 活动日志 | 全站操作审计，变更可追溯                                             |
...
- [ ] 以管理员账号进入「系统设置」，保存 LOGO 与标题后全站、登录 / 注册页同步生效
...
- 品牌配置（LOGO / 标题）属全站公开读取项（未登录的登录页也需展示），请勿在其中填写敏感信息
...
- 备份导出 / 导入会包含品牌配置（LOGO 以 base64 存于数据库），换机迁移时配置随备份一并恢复
...
- 首次启动会自动建表；项目未接入 Alembic 迁移工具，**新增表**会随启动自动创建，而已有表的字段变更需手工执行 SQL（变更前务必先备份）
...
- 系统 LOGO、系统标题与登录 / 注册页文案统一在「系统设置」维护（仅管理员），保存后全站即时生效；「恢复默认」可一键还原为内置品牌信息
