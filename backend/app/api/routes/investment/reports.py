...
from app.models import InvestmentReport
...
def list_reports(db: Session = Depends(get_db)):
    rows = db.scalars(select(InvestmentReport).order_by(InvestmentReport.id.desc())).all()
    return [
        {
            "id": r.id,
            "title": r.title,
...
def generate_report(month: str | None = Query(None, description="YYYY-MM，默认当前月"), db: Session = Depends(get_db)):
    ...
    title, summary, content = build_investment_report(db, month or label)
...
def get_report(report_id: int, db: Session = Depends(get_db)):
    r = db.get(InvestmentReport, report_id)
    if not r:
