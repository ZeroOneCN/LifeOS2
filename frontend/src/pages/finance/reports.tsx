...
export function FinanceReportsPage() {
  ...
  const { confirm, dialog: confirmDialog } = useConfirm({
    title: '确认删除报告',
    description: '确定删除这份财务报告吗？此操作不可恢复。',
  })

  const load = async () => {
    setLoading(true)
    try {
      const res = await api.query<ReportItem[]>('/finance/reports')
      setItems(res)
    } catch (e) {
  ...
  const loadDetail = async (id: number) => {
    try {
      const rep = await api.get<ReportDetail>('/finance/reports', id)
      setPreview(rep)
  ...
  const handleGenerate = async () => {
    setGenerating(true)
    try {
      const rep = await api.post<ReportDetail>('/finance/reports/generate', period)
      setGenOpen(false)
  ...
  const handleExport = async (id: number) => {
    setExportingId(id)
    try {
      await api.download(`/finance/reports/${id}/export`)
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await api.remove('/finance/reports', id)
      toast.success('报告已删除')
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-heading text-2xl font-semibold tracking-tight">财务报告</h1>
          <p className="text-sm text-muted-foreground">
            自动汇总各财务模块数据，支持近 7/30/90 天或自定义区间生成报告并导出 PDF。
          </p>
        </div>
      ...
      <Dialog open={genOpen} onOpenChange={setGenOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>生成财务报告</DialogTitle>
            <DialogDescription>
              选择统计区间（近 7/30/90 天或自定义起止日期），系统将自动汇总区间内财务数据并生成报告。
            </DialogDescription>
          </DialogHeader>
          <div className="py-2">
